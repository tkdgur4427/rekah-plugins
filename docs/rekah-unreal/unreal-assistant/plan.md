...
### 수동 테스트 필요 (실제 스킬 호출 시)

- [ ] Claude Code에서 스킬 목록에 표시됨
- [ ] **경로 질문: 클래스 - 경로 누락 시 최종 경로 질문**
- [ ] **경로 질문: 모듈 - 경로 누락 시 플러그인 경로 질문**
- [ ] **경로 질문: 플러그인 - 경로 누락 시 생성 위치 질문**
- [ ] **경로 질문: 경로 포함 시 질문 없이 바로 진행**
- [ ] UObject 클래스 생성 동작
- [ ] Raw 클래스 생성 동작
- [ ] GenerateProjectFiles.bat 실행됨
- [ ] 중복 파일 경고 동작
- [ ] 경로 검증: 잘못된 경로 시 에러 메시지 출력
- [ ] (선택) 생성된 코드 빌드 성공
